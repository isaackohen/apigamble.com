...
class PaymentTatumController extends Controller
{
	...
	public function getBal($currency, $address) 
{
...
if ($err) {
  ...
  return $response;
	...
	public function getBSCbyTX($hash) 
{
	...
	public static function sendBSC($currency, $from, $privkey, $to, $amount) 
{
	...
	public function sendBEP20Token($currency, $from, $privkey, $to, $amount) 
{
		if($currency === 'xBLZD') {
...
if ($err) {
  ...
  return $response;
	...
	public function sendBitcoincash($mnemonic, $xpub, $accountid, $to, $amount) 
{
$curl = curl_init();
...
if ($err) {
  ...
  echo $response;
	...
	public function estimateBitcoin($mnemonic, $xpub, $accountid, $to, $amount) 
{
$curl = curl_init();
...
if ($err) {
  ...
  return $response;
	...
	public function sendBitcoins($mnemonic, $xpub, $accountid, $to, $amount) 
{
$curl = curl_init();
...
if ($err) {
  ...
  echo $response;
	...
	public function sendLitecoin($mnemonic, $xpub, $accountid, $to, $amount) 
{
$curl = curl_init();
//18Vcjf1yRLrho49VSCiBKSg7nNoJu5aE9j
...
if ($err) {
  ...
  return $response;
	...
	public function sendDogecoin($mnemonic, $xpub, $accountid, $to, $amount) 
{
$curl = curl_init();
//18Vcjf1yRLrho49VSCiBKSg7nNoJu5aE9j
...
if ($err) {
  ...
  return $response;
	...
	public function sendTransfer($currency, $from, $privkey, $to, $amount) 
	{
...
if ($err) {
  ...
  return $response;
	...
	public function transferBTC($tatumaccount, $xpub, $amount, $mnemonic, $to) 
	{
	...
	public function createTatumAddress($tatumaccount, $index, $apikey) 
	{
		$curl = curl_init();
	...
	public function createAddress($apikey, $currency, $label) 
	{
		...
		if($currency === 'bsc' || $currency === 'xblzd' || $currency === 'game1' || $currency === 'betshiba' || $currency === 'gamblecoin' || $currency === 'ltc' || $currency === 'btc' || $currency === 'bch'  || $currency === 'doge' || $currency === 'tron' || $currency === 'trx' || $currency === 'eth') {
			$getindex = DB::table('Wallets')->count() + 13 + 1;
			$contractaddress = 'no';
